...
async function loadFeatured() {
  try {
    // Carrega modelos pré-definidos do JSON
    ...
    // Para o index, mostra PRIMEIRO os pré-definidos, DEPOIS os criados
    const allModels = [...predefinedModels, ...userModels];
    ...
    // Fallback: só mostra os do localStorage
    ...
    if (carousel) {
      ...
      models.forEach(model => {
        ...
        card.className = "carousel-item";
        card.innerHTML = `
          <div class="carousel-thumb" style="background-image:url('${model.image || 'assets/placeholder.jpg'}')"></div>
          <h3>${model.title}</h3>
...
function loadStoreProfessionals() {
  ...
  if (professionals.length === 0) {
    carousel.innerHTML = '<div class="carousel-item professionals" style="text-align: center; padding: 2rem;">Nenhum profissional cadastrado ainda. <a href="publicar-modelo.html" style="color: var(--accent);">Cadastrar agora</a></div>';
    return;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (filename === 'index.html' || filename === '') {
    // Página inicial - mostra pré-definidos + criados
    console.log('Carregando index.html');
    loadFeatured();
    loadProfessionals(); // Esta versão carrega pré-definidos + criados
  } else if (filename === 'store.html') {
    // Página da loja - mostra APENAS criados
    console.log('Carregando store.html');
    loadStoreModels();
    loadStoreProfessionals(); // APENAS criados
  } else if (filename === 'model.html') {
    // Página de detalhe do modelo
